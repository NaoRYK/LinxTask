...
const EditProjectForm = ({ project, onEditProject, onClose }) => {
  const [projectText, setProjectText] = useState(project.name || "");
  const [projectColor, setProjectColor] = useState(project.color || "#FFD1D1");
  const [users, setUsers] = useState([]);
  const [filteredUsers, setFilteredUsers] = useState([]);
  const [selectedCollaborators, setSelectedCollaborators] = useState(project.collaborators || []);
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <div className="bg-[#71E4B9] w-[600px] h-[65px] rounded-[20px] flex shadow-md items-center justify-center gap-2">
          <label
            className="text-[20px] font-bold text-[#09926C]"
            htmlFor="projectName"
          >
            Nombre del proyecto
          </label>
        ...
        <div className="flex items-center gap-2 mr-16">
          ...
          <label
            ...
            className="text-[#09926C] text-[14px] font-bold w-[149px]"
          >
      ...
      <div className="mt-4 bg-[#71E4B9] w-[560px] h-[277px] rounded-[20px] p-4">
        <label
          ...
          className="block text-[#09926C] text-[20px] font-bold mb-2"
        >
              ...
              <option
                ...
                className={`bg-[#A9F1D2] ${
                  selectedCollaborators.includes(user.id)
                    ? "text-tertiaryGreen"
                    : "hover:bg-[#71E4B9] hover:text-[#09926C]"
                }`}
              >
                {user.name}
              </option>
      ...
      <div className="w-full flex justify-end gap-2">
        <button
          className="bg-[#18b183] w-[140px] h-[60px] rounded-[100px] shadow-md text-[#077559] font-semibold mt-4"
          onClick={onClose}
        >
          Cerrar
        </button>
        <button
          className="bg-[#17CF97] w-[140px] h-[60px] rounded-[100px] shadow-md text-[#077559] font-semibold mt-4"
          type="submit"
        >
          Guardar cambios
        </button>
